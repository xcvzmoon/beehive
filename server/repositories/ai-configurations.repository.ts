import type {
  AiConfiguration,
  InsertAiConfigurationInput,
} from '~/server/types/ai-configuration.ts';
...
export function selectAiConfigurationOfDefaultWorkspace(
  workspaceId: AiConfiguration['workspaceId'],
) {
  return databaseEffect({
    table: 'ai-configurations',
    callback: 'selectAiConfigurationOfDefaultWorkspace',
    execute: async () => {
...
export function insertAiConfigurationDefault(input: InsertAiConfigurationInput) {
  return databaseEffect({
    table: 'ai-configurations',
    callback: 'insertAiConfigurationDefault',
    execute: async () => {
      return db.transaction(async (tx) => {
        ...
        const [inserted] = await tx.insert(aiConfigurations).values(input).returning();

        logger.success(`Inserted AI configuration -> ${inserted.id}`);
